...
public class Simpsons{
    ...
    private Simpsons ageType(){
        Property age = model.createProperty(prefix("age", model.getNsPrefixURI("foaf")));
        Iterator<Statement> statements = model.listStatements((Resource) null, age, (Resource) null);
    ...
    public Simpsons createFile(String file){
        try (PrintWriter pw = new PrintWriter(file)){
            // Dynamically determine serialization format based on file extension
            String fileExtension = getFileExtension(file);
            model.write(pw, fileExtension);
        } catch (Exception e){
    ...
    private String getFileExtension(String file) {
        ...
        if (lastIndex != -1 && lastIndex < file.length() - 1) {
            return file.substring(lastIndex + 1);
        }
        // Default to Turtle serialization if extension not found
        return "Turtle";
    }
